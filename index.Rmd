...
ROC_curve = function(probs,response,cut=.5){
  ...
  tpr = sapply(cutoffs, function(cut) sum((probs >= cut) == T & response == "Hääletas")/(sum(response == "Hääletas")))
  fpr = sapply(cutoffs, function(cut) sum((probs >= cut) == T & response == "Ei hääletanud")/(sum(response == "Ei hääletanud")))
