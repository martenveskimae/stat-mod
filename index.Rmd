...
- <a href="http://www.damienfrancois.be/blog/files/modelperfcheatsheet.pdf">Model performance cheatsheet</a>

Logistiline regressioon:

- <a href="https://en.wikipedia.org/wiki/Exponential_family">Eksponentide perekond</a>
- <a href="https://cran.r-project.org/web/packages/margins/vignettes/Introduction.html">An Introduction to ‘margins’</a>
- <a href="https://cran.r-project.org/web/packages/interplot/vignettes/interplot-vignette.html">interplot: Plot the Effects of Variables in Interaction Terms</a>
...
### Andmestik
```{r, include=F, echo=F}
# RK_2015$wage = recode(RK_2015$wage,
#                       "<200 eur" = "under 200 eur",
#                       ">2500 eur" = "over 2500 eur")
# RK_2015 = RK_2015 %>%
#   mutate(vote = recode(vote, "Hääletas" = "voted", .default = "abstained"),
#          gender = recode(gender, "Naine" = "female", .default = "male"),
#          education = recode(education,
#                             "Kõrgharidus" = "higher",
#                             "Keskharidus" = "secondary",
#                             "Põhiharidus" = "primary"),
#          ethnicity = recode(ethnicity, "Eestlane" = "estonian", .default="other"),
#          wage = recode(wage,
#                        "Alla 200 €" = "<200 eur",
#                        "200–275 €" = "200–275 eur",
#                        "276–350 €" = "276–350 eur",
...
#                        "501–575 €" = "501–575 eur",
#                        "576–700 €" = "576–700 eur",
#                        "701–1000 €" = "701–1000 eur",
#                        "1001–1300 €" = "1001–1300 eur",
#                        "1301–1600 €" = "1301–1600 eur",
#                        "1601–1900 €" = "1601–1900 eur",
#                        "1901–2200 €" = "1901–2200 eur",
#                        "2201–2500 €" = "2201–2500 eur",
#                        "Üle 2500 €" = ">2500 eur"))
# 
# write.csv(RK_2015, "RK_2015.csv", row.names=F)
# RK_2015 = read.csv("RK_2015.csv")
```
...
```{r}
voted = c("abstained","voted")
wages = c("under 200 eur","200–275 eur","276–350 eur","351–425 eur","426–500 eur","501–575 eur",
           "576–700 eur","701–1000 eur","1001–1300 eur","1301–1600 eur","1601–1900 eur",
           "1901–2200 eur","2201–2500 eur","over 2500 eur")
edu = c("primary","secondary","higher")

RK_2015 = RK_2015 %>%
  mutate(vote = factor(vote, levels=voted),
         wage = factor(wage, levels=wages),
         wage_numeric = as.numeric(wage),
         education = factor(education, levels=edu))
...
```{r}
f = formula(vote ~ age + I(age^2) + gender + education + ethnicity + wage_numeric)
...
Šansside suhet ei saa siiski lineaarselt tõlgendada, sest konkreetne šansi suurus sõltub X väärtustest. Üks viis, kuidas kõikide X peale keskmist mõju suurust väljendada, on leida keskmine marginaalefekt (*average marginal effect*, AME). AMEde leidmiseks on kõige mugavam kasutada funktsiooni `margins(...)` paketist **margins**. Margins arvutab vaikimisi mõju tõenäosusele. Kui soovida algseid *link* väärtuseid, tuleb määrata `type="link"`.

```{r}
m = margins(osalus_mudel)
summary(m)
```

Kui mudelis on interaktsioonid (antud juhul on mudelis vanuse interaktsioon iseendaga), siis interaktsioonide efektid integreeritakse kokku. Et näha, kas interaktsioonil oli oluline mõju, võib arvutada marginaalefektid kindlatel vanuse väärtustel. Leiame vanuse marginaalefektid vanuse 20 aastase sammuga `at = list(age = seq(20,90,20))`.

```{r}
margins(osalus_mudel, at = list(age = seq(20,90,20)))
```

`cplot(...)` samast paketist võimaldab graafida erinevaid tunnuseid, sh interaktsioone.
```{r}
cplot(osalus_mudel, "age")
```

**Margins** paketi võimaluste kohta saab rohkem infot siit: <a href="https://cran.r-project.org/web/packages/margins/vignettes/Introduction.html">An Introduction to ‘margins’</a>

<div class="tip">

AMEde käsitsi arvutamiseks tuleb leida esmalt keskmine link väärtuste tihedus ning korrutada see koefitsientide suurustega. Kui see tundub praegu keeruline, siis antud hetkel polegi tarvis seda mõista. Huvi korral võib alustada tõenäosuse ja link uurimist siit: <a href="http://thomasleeper.com/Rcourse/Tutorials/binaryglmplots.html">Binary Outcome GLM Plots</a>.

```{r}
(ame = mean(dlogis(predict(osalus_mudel, type="link"))) * coef(osalus_mudel))
```

Link väärtuste tihedus näeb välja järgnev:

```{r}
plot(predict(osalus_mudel, type="link"), dlogis(predict(osalus_mudel, type="link")))
```
...
Mudeli hindamise võib jaotada laias laastus kaheks: (1) mudeli ennustusvõime ja (2) mudeli sobivus. Esimene neist hindab, kui hästi suudab mudel X-ide abil Y väärtuseid hinnata ning teine, kas mudelit oleks võimalik täiustada (näiteks muutes mudeli spesifikatsiooni või funktsionaalset vormi). Mudeli ennustusvõimega seotud indikaatorid on (enamasti 0-1 vahel) meetrikud ning sobivusega seotud indikaatorid statistilised testid p-väärtustega.

## Mudeli ennustusvõime

### Pseudo R-ruut

Kui lineaarse regressiooni puhul sai arvutada R-ruudu, mis näitas X-ide ja Y kovarieeruvust, siis logistilise regressiooni puhul seda otseselt teha ei saa. Küll aga on olemas pseudo R-ruudud. Pseudo R-ruudu leidmiseks on tosinkond erinevat viisi, mis annavad mõnevõrra erinevaid tulemusi. Praegu arvutame McFaddeni pseudo R-ruudu, mis on üks levinumaid pseudo R-ruudu meetodeid. Arvestada tuleks, et erinevalt lineaarse regressiooni R-ruudust, on pseudo R-ruudu suurus reeglina madalam (0.2-0.4 on juba hea tulemus). McFaddeni pseudo R-ruudu valem on:

$R^2=1-\frac{logLik_{mod}}{logLik_{null}}$

ehk see näitab spetsifitseeritud mudeli ja null mudeli logaritmitud tõepärade suhet.
...
<div class="tip">
Meenutame, et andmestikus on hääletanuid oluliselt rohkem kui mittehääletanuid. Mudelit saaks seega parandada tasakaalustatud andmestikuga. Kasutame selleks paketti **ROSE** ja funktsiooni `ovun.sample(...)`.
...
Klassifikatsiooni tabel (vahel ka *confusion matrix*) on olemuselt risttabel, kus ühel teljel on tegelike väärtuste sagedused ning teisel teljel ennustatud väärtused. Klassifikatsiooni tabelist on võimalik arvutada mitmeid kasulikke meetrikuid mudeli hindamiseks, millest tavapärasemad on *accuracy*, *precision*, *true positive rate* (TPR; vahel *sensitivity* või *recall*), *false positive rate* (FPR), *true negative rate* (TNR; vahel *specificity*) ja *F-measure*.

Klassifikatsiooni tabeli formaat on järgnev (telgede järjestust võib muuta):

Tegelik/Ennustatud  | Positiivne (1)     | Negatiivne (0)     | Kokku
--------------------|--------------------|--------------------|-----------------
Positiivne (1)      | True Positive (TP) | False negative (FN)| TP + FN
Negatiivne (0)      | False Positive (FP)| True negative (TN) | FP + TN
Kokku               | TP + FP            | FN + TN            | N
...
- $FPR = \frac{FP}{FP+TN}$
- $TNR = \frac{TN}{FP+TN}$
- $F-measure = \frac{2}{\frac{1}{Precision}+\frac{1}{TPR}}$

Antud väärtusi saab tõlgendada järgnevalt:

- accuracy: mudel suutis õigesti klassifitseerida ...\% vaatlustest
- precision: kõikidest positiivsetest hinnangutest olid õiged ...\% vaatlusi
- TPR: kõikidest tõelistest positiivsetest vaatlustest suutis mudel õigesti klassifitseerida ...\% vaatlusi
- FPR: tõenäosus lükata vääralt kõrvale null hüpotees (sarnane tüüp I vea mõistele)
- TNR: kõikidest tõelistest negatiivsetest vaatlustest suutis mudel õigesti klassifitseerida ...\% vaatlusi
- F-measure võtab arvesse nii *precisionit* kui *TPRi*, karistades mudeleid, mis annavad kallutatud tulemusi

Antud meetrikute harjutamiseks võib need käsitsi kalkuleerida valimisosaluse mudeli põhjal.

Reference/Prediction  | Voted (1)     | Abstained (0)     | Total
----------------------|---------------|-------------------|-----------------
Voted (1)             | 639           | 7                 | 646
Abstained (0)         | 141           | 9                 | 150
Total                 | 780           | 16                | 796

Klassifikatsiooni tabeli ning mitmeid meetrikuid saab funktsiooniga `confusionMatrix(...)` paketist **caret**. Funktsiooni läheb sisendiks mudeli ennustatud väärtused ning tõelised väärtused. Mudeli ennustatud väärtuste saamiseks tuleks valida sobiv lõikepunkt tõenäosuste jaotuses ning klassifitseerida vaatlused.

```{r}
logit_preds = ifelse(fitted(osalus_mudel)>.5,"voted","abstained")
...
ROC-kõver näitab TPR-i ja FPR-i suhet erinevate tõenäosuste lõikepunktide vahel. Lisaks võimaldab ROC-kõver visuaalselt hinnata mudeli accuracy-t.

```{r}
ROC_curve = function(probs,response){
  ...
  auc = auc_roc(probs, ordered(response))
  
  stats %>%
    ggplot() +
    geom_line(aes(fpr,tpr)) +
    geom_ribbon(aes(x=fpr,ymax=tpr,ymin=0),alpha=.5,fill="steelblue") +
...
ROC_curve(fitted(osalus_mudel), RK_2015$vote)
```
Kiire ROC-kõvera saab joonistada paketiga **ROCR**. Selleks on vaja mudelist saadud tõenäosused ja tõelised väärtused siduda kokku `prediction(...)` objektiks, teisendada see `performance(...)` objektiks ning seejärel `plot(...)` funktsiooniga tulemus visualiseerida.

```{r}
pred = prediction(fitted(osalus_mudel), RK_2015$vote)
...
Üks levinumaid sobivusteste on Hosmeri ja Lemeshow test. HL test jaotab mudelist saadud vaatluste tõenäosused esmalt kvantiilidesse ning hindab seejärel, kas kvantiilide `y=1` proportsioonid vastavad tegelikele proportsioonidele. $H_0$ on antud testi puhul sarnased proportsioonid, ehk hea mudeli korral on testi tulemus kõrge p-väärtusega (näitab, et mudel sobitub andmetega hästi). HL testi saab paketist **ResourceSelection** funktsiooni abil `hoslem.test(...)`, kuhu lähevad argumentideks `x = õiged väärtused (0 või 1)` ja `y = mudelist saadud tõenäosused`.

```{r}
hoslem.test(x=osalus_mudel$y, y=fitted(osalus_mudel))
```

# Veel teemasid

Veel statistilise modelleerimise teemasid, mida sissejuhatavas kursuses ei kaetud:

- Mudelite treenimine, valideerimine ja testimine
- Mudeli robustsus
- Instrumenttunnused ja instrumentmudelid
- Mitte-representatiivsed andmed
- Loendusandmete analüüs
- Aegridade analüüs
