...
<div class="tip">
Lisaks mediaanile võivad meid huvitada ka teised **protsentiilid** (mediaan on 0.50 protsentiil), eeskätt .25 ja 0.75 (teisisõnu I ja III kvartiil). Veel võite kohta mõistet **kvartiilide vahe**, mis tähistab vahemikku I ja III kvartiili vahel. R-is on protsentiilide leidmiseks käsklus `quantile(...)`.
...
```{r, eval=FALSE}
toetus.sim = function(x,party,ylemineaalumine){
  require(mosaic)
  ...
  tmp = list(y = ci[["97.5%"]],
             a = ci[["2.5%"]])
  if(ylemineaalumine=="y") return(tmp[["y"]])
  else if(ylemineaalumine=="a") return(tmp[["a"]])
}
