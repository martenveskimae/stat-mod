...
$\rho=\frac{\sum\limits_{i=1}^n(x_{i}-\bar{x})(y_{i}-\bar{y})}{(n-1)s_{x}s_{y}}$

Et anda hinnang $\rho$ statistilisele olulisusele, võime teha t-testi, kus hindame $\rho$ erinevust nullist. T-skoori saamiseks:
...
rho = sum((x-xbar)*(y-ybar)) / ((n-1)*sx*sy)
rho
```
...
```{r}
t = (rho*sqrt(n-2)) / sqrt(1-rho^2) # t-väärtus
