...
Küsitlustes kasutatakse tihti likert skaalasid, mida visualiseeritakse enamasti tulpdiagrammi abil (skaala on protsentides).
...
Antud joonist saab muuta loetavamaks vahetades värviskaalat, pöörates joonise teljed ümber, muutes telgede nimesed jne.
...
Vaatame kahe tunnuse vahelist korrelatsiooni, antud juhul Pearsoni korrelatsiooni. Kuidas on seotud näiteks Reformierakonna toetus tööpuudusega, skp kasvuga ja inflatsiooniga?
...
Vaikimisi annab antud funktsioon nii korrelatsioonimaatriksi kui p-väärtused. Kui soovida vaid ühte, tuleks käskluse lõppu lisada kas `$r` või `$p` korrelatsioonide või p-väärtuste saamiseks. Alati on mõistlik hinnata pilti graafiliselt hajuvusdiagrammidega.

**To do: korrelatsiooni joonis, spearmani korrelatsioon**
```{r, include=FALSE}
# corr.test(ref)$r %>%
#   melt() %>%
#   ggplot() +
